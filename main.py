...
from monai.transforms import (
    ...
    Orientationd, Spacingd, CenterSpatialCropd,SpatialPadd, RandFlipd, RandRotated, RandSpatialCropd, RandShiftIntensityd, SpatialCropd
)
...
transforms_labels = Compose([
    LoadImaged(keys=["label"]),  # Load the label with the key 'label'
    EnsureChannelFirstd(keys=["label"]),
    Orientationd(keys=["label"], axcodes="RAS"),
    CenterSpatialCropd(keys=["label"], roi_size=[128, 128, 128]),
...
# Prepare the image dictionary for the transform
test_data2 = {'image': test_image_path}
label_data = {'label': test_label_path}

# Apply the transforms
test_data2 = transforms2(test_data2)
...
# model.load_state_dict(torch.load('/work/olivesa/blackmamba/best_model2p1.pth'))

# Model in evaluation mode
model.eval()

# Get the test tensor, ensuring it's on GPU
test_tensor = test_data2['image'].unsqueeze(0).cuda()  # Add a batch dimension
label_tensor = label_data['label'].unsqueeze(0).cuda()  # Add a batch dimension for consistency

# Run inference
output2 = model(test_tensor)

# Convert the output to a numpy array and move it to CPU memory
output_data2 = output2.cpu().detach().numpy()



# Select one channel of the output for simplicity (modify this as needed)
output_channel_data = output_data2[0, 0, :, :, :]  # Select the first channel
...
# threshold = 0.1
# output_channel_data[output_channel_data < threshold] = 0

nrrd.write('inference.nrrd', output_channel_data)
print('okk')


# Extract the label data as a numpy array and apply threshold
label_array = label_tensor.cpu().numpy()
label_channel_data = label_array[0, 0, :, :, :]  # Assuming l

# Save the thresholded label data as an NRRD file
nrrd.write('labels1.nrrd', label_channel_data)
